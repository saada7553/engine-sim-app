...
enum EngineDesignExpander {
    ...
    static func expand(_ intent: EngineIntent) -> EngineSpec {
        ...
        let displacementL = (intent.displacementL ?? defaultDisplacement(layout: layout, intent: intent))
    ...
    private static func applyGeometry(_ spec: inout EngineSpec, intent: EngineIntent, cylVolCc: Double, n: Double) {
        var ratio: Double
        switch (intent.powerBand, intent.camProfile) {
        ...
        spec.chamberVolumeCc = (cylVolCc / (cr - 1.0)).clamped(to: Limit.chamberVolumeCc)
    }
    ...
    private static func applyCam(_ spec: inout EngineSpec, intent: EngineIntent) {
        var (dur, lift, lsa, adv) = camBase(intent.camProfile)
    ...
    private static func applyBreathing(_ spec: inout EngineSpec, intent: EngineIntent, displacementL: Double, cylVolCc: Double) {
        ...
        spec.intakeCfm = (displacementL * 145 * camFlow * boostMul).clamped(to: Limit.intakeCfm)
    ...
    private static func defaultRedline(_ intent: EngineIntent) -> Double {
        var base: Double
        switch intent.camProfile {
        case .economy: base = 6000
        case .stock:   base = 6800
        case .sport:   base = 7800
        case .race:    base = 8800
        }
        switch intent.vehicleClass {
        case .motorcycle:           base += 2000
        case .muscle, .truck:       base -= 800
        case .supercar, .raceCar:   base += 800
        default: break
        }
        return base
    }
    ...
    private static func peakAdvance(_ fuel: FuelPreset) -> Double {
        switch fuel {
        case .gasoline: return 36
        case .e85, .methanol: return 38
        case .diesel: return 22
        }
    }
